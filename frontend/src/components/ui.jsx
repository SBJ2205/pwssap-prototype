...
export function ErrorState({ message, onRetry }) {
  return (
    <Card>
      <div style={{ color: "#791F1F", fontSize: 13, marginBottom: onRetry ? 10 : 0 }}>
        ✕ {message || "Something went wrong."}
      </div>
      {onRetry && (
        <button onClick={onRetry}
          style={{ padding: "6px 14px", background: "#791F1F", color: "#fff", border: "none", borderRadius: 6, cursor: "pointer", fontSize: 12, fontWeight: 500 }}>
          Retry
        </button>
      )}
    </Card>
  ...
  return (
    <Card>
      <div style={{ color: "#888", fontSize: 13 }}>
        {title && <div style={{ fontWeight: 500, marginBottom: 4, color: "#555" }}>{title}</div>}
        {message}
      </div>
    </Card>
  );
