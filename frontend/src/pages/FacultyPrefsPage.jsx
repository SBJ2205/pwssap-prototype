...
export default function FacultyPrefsPage({ students, subjects, facultyBySubject, selStudent, setSelStudent, facultyPrefs, cycleFacultyRating, saveFacultyPrefs, lockedStudentId }) {
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "200px 1fr", gap: 14 }}>
        <div>
          ...
          <Card>
            <div style={{ fontSize: 12, color: "var(--text-secondary)", marginBottom: 8 }}>Summary</div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span style={{ fontSize: 11, color: "var(--text-secondary)" }}>Faculty rated</span>
              <span style={{ fontSize: 12, fontWeight: 600, background: "var(--rating-blue-bg)", color: "var(--rating-blue-fg)", padding: "1px 8px", borderRadius: 10 }}>{ratedCount} / {rankableSlotCount}</span>
            </div>
          </Card>
        </div>
            ...
            return (
              <Card key={code}>
                <div style={{ fontWeight: 500, marginBottom: 2 }}>{subjectNames[code] || code}{" "}
                  <code style={{ fontSize: 11, background: "var(--bg-surface-alt)", padding: "1px 5px", borderRadius: 3 }}>{code}</code>
                </div>
                  ...
                  return (
                    <div key={t.id} onClick={() => cycleFacultyRating(code, t.id)}
                      style={{
                        ...
                        background: m.bg, border: `1.5px solid ${rating > 0 ? m.color : "var(--border-subtle)"}`,
                      }}>
