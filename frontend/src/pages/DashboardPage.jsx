...
export default function DashboardPage({ results }) {
  ...
  const baselines = [
    { label: "PWSSAP (ours)", val: total, color: "var(--accent-green)" },
    { label: "FCFS baseline", val: fcfs ? fcfs.total_penalty : 0, color: "var(--accent-blue)" },
    { label: "Random baseline", val: random ? random.total_penalty : 0, color: "var(--accent-orange)" },
  ];
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
        <Card>
          <div style={{ fontWeight: 500, marginBottom: 4 }}>Penalty vs baselines{" "}
            <Tooltip text={"Comparison against two naive approaches:\n\n• FCFS (First Come First Served): assigns sections in the order students registered — no preference awareness.\n\n• Random: assigns sections randomly — purely by chance.\n\nLower bar = fewer unhappy slots."}>
              <span style={{ fontSize: 11, color: "var(--text-muted)", cursor: "help", borderBottom: "1px dashed var(--text-muted)" }}>what are baselines?</span>
            </Tooltip>
          </div>
          <div style={{ fontSize: 12, color: "var(--text-muted)", marginBottom: 12 }}>Lower bar = better outcome</div>
          {baselines.map(b => (
            <div key={b.label} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
              ...
              <div style={{ flex: 1, background: "var(--bg-surface-alt)", borderRadius: 3, overflow: "hidden", height: 22 }}>
                <div style={{ width: (b.val / maxB * 100) + "%", height: "100%", background: b.color, display: "flex", alignItems: "center", paddingLeft: 8, fontSize: 12, color: "var(--on-accent)", fontWeight: 500 }}>
                  {b.val}
                </div>
              </div>
            </div>
        ...
        <Card>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
            <thead><tr>{["Student", "Roll", "Penalty", "Faculty Pen.", "Status"].map(h => <th key={h} style={{ textAlign: "left", padding: "4px 8px", fontSize: 11, color: "var(--text-muted)", borderBottom: "1px solid var(--border-subtle)" }}>{h}</th>)}</tr></thead>
            <tbody>
        ...
        <Card>
          <div style={{ fontWeight: 500, marginBottom: 4 }}>Gap-reduction post-processing{" "}
            <Tooltip text={"After the CP-SAT optimum is found, a second pass looks for idle periods between a student's classes and tries swapping to an alternative section of the SAME subject that removes the gap.\n\nA swap is only applied if it does not increase that student's penalty — so the total penalty can only stay the same or improve, never get worse."}>
              <span style={{ fontSize: 11, color: "var(--text-muted)", cursor: "help", borderBottom: "1px dashed var(--text-muted)" }}>what is this?</span>
            </Tooltip>
          </div>
          <div style={{ fontSize: 12, color: "var(--text-muted)", marginBottom: 10 }}>
          ...
          {results.gap_reduction.changes.length > 0 && (
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
              <thead><tr>{["Student", "Subject", "From → To Section", "Gaps Before → After"].map(h => <th key={h} style={{ textAlign: "left", padding: "4px 8px", fontSize: 11, color: "var(--text-muted)", borderBottom: "1px solid var(--border-subtle)" }}>{h}</th>)}</tr></thead>
              <tbody>
